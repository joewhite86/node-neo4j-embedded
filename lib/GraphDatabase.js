...
GraphDatabase.prototype.beginTx = function() {
  ...
  debug(function() {console.log('transaction started')});
...
GraphDatabase.prototype.createNode = function(label, params) {
  ...
  for(var i = 0; i < args.length; i++) {
    if(typeof args[i] === 'string') labels.push(neo4j.DynamicLabel.labelSync(args[i]));
      ...
      labels = labels.concat(args[i].map(function(label) {return neo4j.DynamicLabel.labelSync(label);}));
    }
...
GraphDatabase.prototype.getLong = function(value) {
  ...
  var ERR = Tools.createErrorHandler();
  try {
    return neo4j.java.newInstanceSync('java.lang.Long', value.toString());
  }
  catch(err) {
...
GraphDatabase.prototype.query = function(query, queryParams, cb) {
  ...
  var that = this,
      params = neo4j.java.newInstanceSync('java.util.HashMap');
  ...
  return new Promise(function(resolve, reject) {
    var ERR = Tools.createErrorHandler(QueryBuilder.prototype.execute), columns, start, end;
    ...
    neo4j.neo4jWrapper.query(that.database, query, params, function(err, queryResult) {
      debug(function() {
        console.log('result received');
      });
      if(ERR.handleError(err, reject)) return;
