...
GraphDatabase.prototype.createNode = function() {
  ...
  return new Node(neo4j, this.database.createNodeSync());
};
...
GraphDatabase.prototype.query = function(query, queryParams, cb) {
  ...
  var rowResult, row, key, value, params = neo4j.java.newInstanceSync('java.util.HashMap');
  ...
  neo4j.neo4jWrapper.query(this.database, query, params, function(err, queryResult) {
    ...
    var columnNames = neo4j.neo4jWrapper.columnNames;

    for(var i = 0; i < queryResult.length; i++) {
      row = queryResult[i];
      rowResult = {};
      for(var j = 0; j < row.length; j++) {
        rowResult[columnNames[j]] = mapResult(row[j]);
      }
...
//   var result = [], rowResult, row, colIter, column, value, params = neo4j.java.newInstanceSync('java.util.HashMap');
