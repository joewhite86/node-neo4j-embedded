 ...
 */
function GraphDatabase(_neo4j, dir, properties, serverProperties) {
  'use strict';

  neo4j = _neo4j;

  try {
    if(serverProperties !== false) this.database = neo4j.neo4jWrapper.connectWrappedSync(dir, objectToHashMap(properties), objectToHashMap(serverProperties));
    else this.database = neo4j.neo4jWrapper.connectSync(dir, objectToHashMap(properties));
...
function objectToHashMap(obj) {
  ...
  var map = neo4j.java.newInstanceSync('java.util.HashMap');

  if(obj && 'object' === typeof obj) {
    for(key in obj) {
      if(obj.hasOwnProperty(key)) {
        params.putSync(key, obj[key]);
      }
    }
  }
