  ...
  if(that._with) {
    if(that._returns.length > 0) {
      ...
      if(that._orderBy.length > 0) {
        query.push('ORDER BY ' + that._orderBy.map(function(sorter) {return sorter.field + ' ' + (sorter.dir||'ASC');}));
      }

      if(that._limit) {
        query.push('SKIP ' + that._limit.skip);
...
QueryBuilder.prototype.getCount = function(params, cb) {
  ...
  return new Promise(function(resolve, reject) {
    if(that._limit && that._limit.limit === 1)  reject(null, [{count: 1}]);
    else if(that._dontCount)                    reject(null, [{count: -1}]);
    else if(that._returns.length === 0)         reject(null, [{count: 0}]);
 ...
 *     @example
 *     var subquery = query.with('COUNT(n) as nodeCount');
 *     subquery.where('nodeCount > 1');
 */
QueryBuilder.prototype.with = function(queryBuilder) {
  this._with = arguments.length === 1? queryBuilder: new QueryBuilder();
  return this._with;
};
