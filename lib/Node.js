 ...
 * @param {String} name Name of the label to apply.
 */
Node.prototype.addLabel = function(name) {
  'use strict';

  var args = Array.prototype.slice.call(arguments);
  for (var i = args.length - 1; i >= 0; i--) {
    this._node.addLabelSync(neo4j.DynamicLabel.labelSync(args[i]));
  };
...
/**
 * Get a list of labels assigned to the node.
 * @returns {Array} Array of labels assigned to the node represented as string.
 */
Node.prototype.getLabels = function() {
  'use strict';

  return neo4j.neo4jWrapper.getNodeLabelsSync(this._node);
};
...
Node.prototype.getProperty = function(key, defaultValue) {
  ...
  try {
    ...
    if(e.toString().indexOf('PropertyNotFoundException') === 0) throw e;
 ...
 * Get relationships from/to this node.
    @example
    var relationships = homer.getRelationships(neo4j.DIRECTION.OUTGOING, 'HATES', 'DOES_NOT_LIKE', 'WHATEVER_ELSE');
 ...
 * Get related nodes from/to this node.
    @example
    var hatedOnes = homer.getRelationshipNodes(neo4j.DIRECTION.OUTGOING, 'HATES', 'DOES_NOT_LIKE');
...
/**
 * Test if a node has a specific label.
 * @param {String} name Name of the label to look for.
 * @return {Boolean} True if the node has this particular label.
 */
Node.prototype.hasLabel = function(name) {
  'use strict';

  return this._node.hasLabelSync(neo4j.DynamicLabel.labelSync(name));
};
 ...
 */
Node.prototype.removeLabel = function(name) {
  'use strict';

  this._node.removeLabelSync(neo4j.DynamicLabel.labelSync(name));

  return this;
};

Node.prototype.removeProperty = function(key) {
  'use strict';

  this._node.removePropertySync(key);
